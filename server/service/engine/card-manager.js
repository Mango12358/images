...
const DDZPoker = function () {
    ...
    this.getCards = function () {
        if (cardsArr.length === 0) {
            this.getShuffleCards();
        }
        return cardsArr
    }

    this.resetCards = function (cards) {
        if (cards !== undefined && cards.length === 54) {
            cardsArr = cards;
        }
    }
    ...
    this.dealCards = function () {
        ...
        var tmpCards = cardsArr.slice(0)
        var leaveCards = tmpCards.splice(Math.floor(Math.random() * tmpCards.length), 3)
        tmpCards.forEach(function (card, index) {
            if (index <= 50) {
                switch (index % 3) {
                    case 0:
                        player1.push(card)
